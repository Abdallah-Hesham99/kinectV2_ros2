...
public:
    KinectApp(int device_id = 0)
        ...
        info_pub_ = this->create_publisher<sensor_msgs::msg::CameraInfo>("/kinect2/camera_info", 10);
    }
...
//camera_info_msg->header.frame_id = "/kinect2_frame";

camera_info_msg->width = 512; // Update with actual width
camera_info_msg->height = 424; // Update with actual height
camera_info_msg->distortion_model = "plumb_bob";
camera_info_msg->d.resize(5, 0.0); // Set distortion parameters (k1, k2, t1, t2, k3)
camera_info_msg->k = {450, 0, 256, 0, 450, 212, 0, 0, 1}; // Intrinsic matrix (fx, 0, cx, 0, fy, cy, 0, 0, 1)
camera_info_msg->r = {1, 0, 0, 0, 1, 0, 0, 0, 1}; // Rectification matrix
camera_info_msg->p = {450, 0, 256, 0, 0, 450, 212, 0, 0, 0, 1, 0}; // Projection matrix
        ...
        if (last_ptr[COLOR]) {
	     ...
	     header.frame_id = "kinect2_frame"; // Set the frame_id to your desired frame
	    header.stamp = rclcpp::Clock().now();
            camera_info_msg->header = header;
            
            
            auto rgb_msg = cv_bridge::CvImage(header, "bgr8", resized8U3).toImageMsg();
	    ...
	    header.stamp = rclcpp::Clock().now();
            camera_info_msg->header = header;
            // Publish depth image
            auto depth_msg = cv_bridge::CvImage(header, "32FC1", depth32F1).toImageMsg();
